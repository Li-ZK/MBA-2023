...
from models.mix import mixup

"""
ablation-Model3
Regression+Mixup+Band Attention
"""
...
class MBA(nn.Module):
    ...
    def __init__(self):
        ...
        self.k_size = 3
        self.band_attention_module = nn.Sequential(
            nn.Conv1d(1, 1, kernel_size=self.k_size, padding=(self.k_size - 1) // 2, bias=False),
            nn.Sigmoid()
        )
    ...
    def forward(self, x):
        x = x.unsqueeze(1)
        w = self.band_attention_module(x)
        z = x * w  # re-weighted

        z = z.squeeze(1)

        y_pred = self.regression_network(z)
...
class LightningModel(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # mixup model hyper-parameter
        alpha = 0.2

        # 进行mixup
        x, target = mixup(x, target, alpha=alpha)
        x = x.float()
        target = target.float()
    ...
    def training_epoch_end(self, training_outputs):
        ...
        alpha = torch.tensor([x["alpha"] for x in training_outputs]).mean()
        ...
        self.log('alpha', alpha, prog_bar=True)
